...
import { isHBS, isHTML, isGlimmerFileExtension, isScript } from './file-types.js';
...
export default class Linter {
  ...
  async verify(options) {
    ...
    if (isGlimmerFileExtension(filePath)) {
      return this.verifyGJS(source);
    }

    if (isHBS(filePath) || isHTML(filePath)) {
      return this.verifyTemplate(source);
    }
    ...
    if (options.checkHbsTemplateLiterals && isScript(filePath)) {
      return this.verifyHbsTemplateLiterals(source);
    }
  ...
  async verifyTemplate(source) {
    let templateInfo = templateInfoForTemplateFile(source);

    return await this._verifyTemplate(templateInfo);
  }

  async verifyHbsTemplateLiterals(source) {
    // TODO: need to finish for this PR to be non-breaking
    return [];
  }

  async verifyGJS(source) {
    ...
    for (let templateInfo of templateInfos) {
      let templateResults = await this._verifyTemplate(templateInfo);
      ...
      try {
        ...
        if (templateInfo.isStrictMode) {
          for (let result of rule.results) {
            const resultCoordinates = contentTagUtils.reverseInnerCoordinates(
              templateInfo,
              result
            );
